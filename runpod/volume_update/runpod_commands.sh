...
cd /workspace  # or use /runpod-volume if that's where your volume is

# Create the updated flux_handler.py
cat > flux_handler.py << 'HANDLER_EOF'
"""
...
class FluxHandler:
    ...
    def load_workflow(self, is_img2img: bool = False, lora_name: str = None) -> Dict:
        ...
        if is_img2img:
            ...
            workflow_file = "/workflows/flux_with_lora.json"
        else:
    ...
    def update_prompt(self, workflow: Dict, prompt: str, width: int = 1024, height: int = 1024, image_data: bytes = None, lora_name: str = None, lora_strength: float = 1.0) -> Dict:
        ...
        if lora_name:
            # Check if LoRA exists, if not try to download it
            lora_path = f"/ComfyUI/models/loras/{lora_name}.safetensors"
            if not os.path.exists(lora_path):
                logger.info(f"LoRA {lora_name} not found locally, checking known LoRAs...")
                ...
                if lora_name in known_loras:
                    ...
                    try:
                        ...
                        urllib.request.urlretrieve(known_loras[lora_name], lora_path)
                        logger.info(f"Successfully downloaded {lora_name}")
                    except Exception as e:
                        logger.error(f"Failed to download LoRA: {e}")
                        # Continue anyway - ComfyUI will show error
            ...
            for node_id, node in workflow.items():
                if node.get("class_type") in ["LoraLoader", "LoraLoaderModelOnly"]:
                    ...
                    logger.info(f"Updated LoRA: {lora_name} with strength {lora_strength}")
...
# Create the LoRA workflow
cat > flux_with_lora.json << 'WORKFLOW_EOF'